...
std::optional<rewheel::gpu_tex2d> texture_manager::wait_texture(const char *path) {
    auto img = coutils::sync::run_join(resldr.load_image(path));
    if (img) { return rewheel::gpu_tex2d(*img); } else { return std::nullopt; }
}

coutils::async_fn<void> texture_manager::texture_to_queue(const char* path) {
...
void texture_manager::want_texture(const std::string& path) {
    ...
    if (not_present) { coutils::sync::unleash(texture_to_queue(path_str->c_str())); }
}

rewheel::gpu_tex2d* texture_manager::get_texture(const std::string& path) {
    want_texture(path);
    return try_get_texture(path);
}

rewheel::gpu_tex2d* texture_manager::try_get_texture(const std::string_view& path) {
