...
cppcoro::generator<glm::mat4> map2d::tex_mats(unsigned int width, unsigned int height) const {
    for (auto&& [block_uv, paste_coord] : pos_parts) {
        co_yield glut::uv_trans(block_uv, width, height, glut::eye4);
    }
}

void map2d::render(game_window& wnd, const position& uv) const {
    ...
    for (size_t i = 0; i < pos_parts.size(); ++i) {
        ...
        auto paste_uv = block_uv - position::offset(block_uv.left, block_uv.top) + paste_coord;
        if (uv.intersect(paste_uv)) {
            auto rel_uv = paste_uv - position::offset(uv.left, uv.top);
